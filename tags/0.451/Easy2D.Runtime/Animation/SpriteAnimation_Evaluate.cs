...
namespace EasyMotion2D
{
    ...
    public partial class SpriteAnimation : MonoBehaviour
    {
        ...
        void UpdateInternal(float delta)
        {
            ...
            {
                //Debug.Log(state.name + ",  " + state.enabled + ",  " + state.weight);
                    ...
                    )
                {
                    ...
                    playingQueue.RemoveAll(delegate(SpriteAnimationState item)
                    {
                        if ( !IsLayerPlaying(item.layer) )
                        {
                            item.time = 0f;
                            item.weight = 1f;
                            item.enabled = true;
            ...
            crossFadeQueue.RemoveAll( delegate(SpriteAnimationQueueItem item)
            {
                SpriteAnimationState fadeState = item.playMode == PlayMode.StopAll ?
                    IsAllLayerCanFade(item.fadeLength)  :
                    IsLayerCanFade(item.state.layer, item.fadeLength);

                if (fadeState != null)
                {
                    CrossFade(item.state, item.fadeLength, item.playMode);
                    return true;
                }
        ...
        private void EvaluateCompoment()
        {
            for (int i = 0; i < spriteRenderer.GetAttachSpriteCount(); i++)
            {
                ...
                foreach (SpriteTransform.StateComponentPair pair in sprTransform.attachStateComponent)
                {
                    ...
                    {
                        if (pair.applyTo.lastChild != null)
                        {
                            string name = pair.state.name + "_" + pair.applyTo.lastRefClip.name;
                            SpriteAnimationState state = animationStates[name] as SpriteAnimationState;
                        ...
                        if (kf.refClip != null)
                        {
                            ...
                            if (!animationStates.Contains(name))
                            {
                    ...
                    //if ( pair.state.blendMode == AnimationBlendMode.Additive )
                    //    Debug.Log( pair.state.clip.name + "/" + pair.component.name + ",  " + weight);
        ...
        private void TransformNode(SpriteTransform node, SpriteTransform parentTransform)
        {
            SpriteTransform sTransform = node;
            float r = 0;
            float sin = 0;
            float cos = 0;
            ...
            for (SpriteTransform iter = node.firstChild; iter != null; iter = iter.next)
            {
                TransformNode(iter, node);
            }
        }
        ...
        SpriteAnimatonDelegate preTransformUpdate = null;
        SpriteAnimatonDelegate postTransformUpdate = null;
